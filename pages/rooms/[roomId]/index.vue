...
const api = useAPI();
const { data: room } = await api.rooms.getRoomItem(roomId);
      ...
      <div class="d-none d-md-block position-relative">
        <div class="d-flex gap-2 rounded-3xl overflow-hidden">
          <div style="width: 52.5vw;">
            <img class="w-100" :src="room?.result.imageUrl" alt="room-a-1">
          </div>
          <div class="d-flex flex-wrap gap-md-2" style="width: 42.5vw;">
            <div class="d-flex gap-md-2">
              <img class="w-50" :src="room?.result.imageUrlList[0]" alt="room-a-2">
              <img class="w-50" :src="room?.result.imageUrlList[1]" alt="room-a-3">
            </div>
            <div class="d-flex gap-md-2">
              <img class="w-50" :src="room?.result.imageUrlList[2]" alt="room-a-4">
              <img class="w-50" :src="room?.result.imageUrlList[3]" alt="room-a-5">
            </div>
          </div>
        </div>
        <button
          ...
          <div class="d-none d-md-block col-md-5">
            <div class="rounded-3xl position-sticky d-flex flex-column gap-10 p-10 ms-auto bg-neutral-0"
              ...
              <h5 class="mb-0 text-primary-100 fw-bold">
                NT$ {{ room?.result.price }}
              </h5>
              <!-- <NuxtLink :to="{ name: 'rooms-roomId-booking', params: { roomId: $route.params.roomId } }"
