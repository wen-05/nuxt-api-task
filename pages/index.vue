...
const { home, rooms } = useAPI();

const { data: getNews } = home.getNews();
const { data: getRooms } = rooms.getRooms();
const { data: getCulinary } = home.getCulinary();

await Promise.allSettled([getNews, getRooms, getCulinary]);
...
<template>
  <main class="overflow-hidden">
      ...
      <div class="container position-relative">
        <div class="row">
          ...
          <div class="col-12 col-md-10 d-flex flex-column gap-10">
            <div v-for="item in getNews?.result" :key="item._id" class="card bg-transparent border-0">
              <div class="d-flex flex-column flex-md-row align-items-center gap-6">
                <picture>
                  <source :srcset="item.image" media="(min-width: 576px)">
                  <img :src="item.image" class="w-100 rounded-3" alt="可看見海景及泳池的套房">
                </picture>
    ...
    <section class="room-intro position-relative px-3 py-20 px-md-0 py-md-30 bg-neutral-120">
      <div
        v-for="item in getRooms?.result" :key="item._id"
        ...
        <swiper ref="roomSwiper" :modules="modules" :slides-per-view="1" :pagination="true" :autoplay="{
          ...
          <swiper-slide v-for="(image, index) in item.imageUrlList" :key="index">
            <picture>
              <source :srcset="image" media="(min-width:768px)">
              <img class="w-100" :src="image" :alt="index">
            </picture>
          </swiper-slide>
        </swiper>
    ...
    <section class="delicacy position-relative py-20 py-md-30 bg-primary-10">
      <div class="container">
        ...
        <div class="row flex-nowrap overflow-x-auto">
          <div v-for="item in getCulinary?.result" :key="item._id" class="col-10 col-md-6 col-xl-4">
              ...
              <div class="card-body position-absolute bottom-0 p-4 p-md-6 rounded-bottom-3  text-neutral-0">
                <div class="d-flex justify-content-between align-items-center mb-4 mb-md-6">
                  ...
                  <div class="d-flex justify-content-between gap-4 text-neutral-40 fs-8 fs-md-7">
                    <span class="fw-bold">{{ item.diningTime.split(' ')[0] }}</span>
                    <span class="fw-bold">{{ item.diningTime.split(' ')[1] }}</span>
                  </div>
                </div>
